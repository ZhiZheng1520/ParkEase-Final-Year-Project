...
db_config = {
    'host': 'tanzhizheng1520.mysql.pythonanywhere-services.coms',
...
def get_available_slots():
    try:
        ...
        if connection.is_connected():
            ...
            # Query to get the total number of slots per zone for APU Carpark B
            cursor.execute("""
                SELECT zone, COUNT(*) AS total_slots
                FROM parking_slots
                WHERE location = 'APU Carpark B'
                GROUP BY zone;
            """)
            total_slots_per_zone = cursor.fetchall()

            # Query to get the number of reserved slots per zone for APU Carpark B
            cursor.execute("""
                SELECT ps.zone, COUNT(r.slot_id) AS reserved_slots
                FROM reservations r
                JOIN parking_slots ps ON r.slot_id = ps.slot_id
                WHERE ps.location = 'APU Carpark B' AND r.status = 'Reserved'
                GROUP BY ps.zone;
            """)
...
def available_slots(location):
    # Fetch available slots for the given location (e.g., 'APU Carpark B')
    slots = get_available_slots(location)
    if slots:
